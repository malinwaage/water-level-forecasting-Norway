...
#files.download('/content/drive/My Drive/models/my_trained_model_water_level_Sogndalsvatn_April2024.keras')

@st.cache_resource
def load_model():
    model = tf.keras.models.load_model('my_trained_model_water_level_Sogndalsvatn_April2024.keras') # Load from the current directory
    return model
...
def plot_predictions(dataset, y_pred):
    future_date_range = pd.date_range(start=dataset.index[-1], periods=FORECAST_HORIZON + 1, freq='3H')[1:]
    plot_df = pd.DataFrame({'Predicted': y_pred[-1]}, index=future_date_range)

    fig = go.Figure()
    fig.add_trace(go.Scatter(x=dataset.index, y=dataset['inflow'], mode='lines', name='Past Water Level', line=dict(color='blue')))
